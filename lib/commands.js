...
module.exports = {
    ...
    say: function say(channel, message) {
        ...
        if ((message.startsWith(".") && !message.startsWith("..")) || message.startsWith("/") || message.startsWith("\\")) {
            ...
            if (message.substr(1, 2) === "w ") {
                var username_message = message.substr(3);
                var usernameEnd = username_message.indexOf(' ');
                if (usernameEnd > -1) {
                    var username = username_message.substr(0, usernameEnd);
                    message = username_message.substr(usernameEnd + 1);
                    return this.whisper(username, message);
                }
            }
    ...
    whisper: function whisper(username, message) {
        ...
        return this._sendCommand(this._getPromiseDelay(), "#jtv", `/w ${username} ${message}`, (resolve, reject) => {
            ...
            userstate = _.merge(userstate, parse.emotes({ emotes: parse.transformEmotes(emotes) || null }));
