...
client.prototype.handleMessage = function handleMessage(message) {
    if (!_.isNull(message)) {
        var channel = _.unchannel(_.get(message.params[0], null));
        ...
        if (message.tags) {
            for(var key in message.tags) {
                if (message.tags.hasOwnProperty(key)) {
                    // This seems like a risky way of doing this. The default behavior is to silently break on new features
                    // (e.g. the addition of the "followers-only" tag).
                    switch(key) {
                        case "emote-sets":
                        case "ban-duration":
                        case "bits":
                            break;
                        case "followers-only": {
                            const duration = parseInt(message.tags[key]);
                            message.tags[key] = duration !== -1 ? duration : false;
                            break;
                        }
                        case "slow": {
                            const duration = parseInt(message.tags[key]);
                            message.tags[key] = duration !== 0 ? duration : false;
                            break;
                        }
                        default:
        ...
        // Messages with "tmi.twitch.tv" as a prefix..
            ...
            switch(message.command) {
                ...
                case "USERNOTICE":
                    if (msgid === "resub") {
                        var systemMsg = _.replaceAll(_.get(message.tags["system-msg"], null), {
                            "\\\\s": " ",
                            "\\\\:": ";",
                            "\\\\\\\\": "\\",
                            "\\r": "\r",
                ...
                // Describe non-channel-specific state information..
                case "GLOBALUSERSTATE":
                    this.globaluserstate = message.tags;

                    // Received emote-sets..
                    var emotesets = message.tags["emote-sets"];
                    if (typeof emotesets !== "undefined") {
                        this._updateEmoteset(emotesets);

                        var sets = [];
                        for(var set of emotesets.split(",")) {
                            sets.push(parseInt(set));
                        }
                        message.tags["emote-sets"] = sets;
                    }

                    this.emit("globaluserstate", message.tags);
                    break;
                ...
                case "ROOMSTATE": {
                    ...
                    const _channel = _.unchannel(message.params[0]);
                    if (_.unchannel(this.lastJoined) === _channel) { this.emit("_promiseJoin", null); }

                    // Provide the channel name in the tags before emitting it..
                    message.tags.channel = _channel;
                    this.emit("roomstate", _channel, message.tags);
        ...
        // Anything else..
            ...
            switch(message.command) {
                ...
                case "WHISPER":
                    ...
                    var from = _.unchannel(message.tags.username);
...
client.prototype._sendMessage = function _sendMessage(delay, channel, message, fn) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!_.isNull(this.ws) && this.ws.readyState !== 2 && this.ws.readyState !== 3 && !_.isJustinfan(this.getUsername())) {
            const _channel = _.unchannel(channel);
            if (!this.userstate[_channel]) { this.userstate[_channel] = {} }
            ...
            this.ws.send(`PRIVMSG ${_channel} :${message}`);
            ...
            var userstate = _.merge(this.userstate[_channel], parse.emotes({ emotes: parse.transformEmotes(emotes) || null }));

            // Message is an action (/me <message>)..
            if (message.match(/^\u0001ACTION ([^\u0001]+)\u0001$/)) {
                userstate["message-type"] = "action";
                this.log.info(`[${_channel}] *<${this.getUsername()}>: ${message.match(/^\u0001ACTION ([^\u0001]+)\u0001$/)[1]}`);
                this.emits(["action", "message"], [
                    [_channel, userstate, message.match(/^\u0001ACTION ([^\u0001]+)\u0001$/)[1], true],
                    [_channel, userstate, message.match(/^\u0001ACTION ([^\u0001]+)\u0001$/)[1], true]
                ]);
            }
            ...
            // Message is a regular chat message..
                ...
                this.log.info(`[${_channel}] <${this.getUsername()}>: ${message}`);
                this.emits(["chat", "message"], [
                    [_channel, userstate, message, true],
                    [_channel, userstate, message, true]
                ]);
            }
            fn(resolve, reject);
...
client.prototype.isMod = function isMod(channel, username) {
    ...
    return this.moderators[_.channel(channel)].indexOf(_.username(username)) >= 0;
};
...
client.prototype.utils = {
    ...
    raffle: {
        ...
        isParticipating: function isParticipating(channel, username) {
            this.init(channel);
            return this.raffleChannels[_.channel(channel)].indexOf(_.username(username)) >= 0;
        }
    },
